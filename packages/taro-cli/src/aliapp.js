...
function transfromNativeComponents (configFile, componentConfig) {
  ...
  if (usingComponents && !Util.isEmptyObject(usingComponents)) {
    Object.keys(usingComponents).map(async item => {
      ...
      const componentWXMLPath = componentJSPath.replace(path.extname(componentJSPath), '.axml')
      const componentWXSSPath = componentJSPath.replace(path.extname(componentJSPath), '.acss')
...
async function buildSinglePage (page) {
  ...
  const outputPageWXMLPath = outputPageJSPath.replace(path.extname(outputPageJSPath), '.axml')
  const outputPageWXSSPath = outputPageJSPath.replace(path.extname(outputPageJSPath), '.acss')
  // 判断是不是小程序原生代码页面
  const pageWXMLPath = pageJs.replace(path.extname(pageJs), '.axml')
  if (fs.existsSync(pageWXMLPath) && pageJsContent.indexOf(taroJsFramework) < 0) {
    ...
    const pageWXSSPath = pageJs.replace(path.extname(pageJs), '.acss')
  ...
  try {
    ...
    Util.printLog(Util.pocessTypeEnum.GENERATE, '页面WXML', `${outputDirName}/${page}.axml`)
    // 编译依赖的脚本文件
    if (Util.isDifferentArray(fileDep['script'], res.scriptFiles)) {
      compileDepScripts(res.scriptFiles)
    }
    ...
    if (Util.isDifferentArray(fileDep['style'], res.styleFiles) || Util.isDifferentArray(depComponents[pageJs], pageDepComponents)) {
      Util.printLog(Util.pocessTypeEnum.GENERATE, '页面WXSS', `${outputDirName}/${page}.acss`)
